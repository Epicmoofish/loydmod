...
public class DreamDimensionDimension extends LoydmodModElements.ModElement {
	@ObjectHolder("loydmod:dream_dimension")
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("dream_dimension"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("loydmod:dream_dimension")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("loydmod:dream_dimension"), dimension, null, true);
		}
		type = DimensionType.byName(new ResourceLocation("loydmod:dream_dimension"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("loydmod:dream_forest")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("loydmod:dream_plains")),};
	}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new OverworldGenSettings() {
				public BlockState getDefaultBlock() {
					return DreamStoneBlock.block.getDefaultState();
				}
			...
			if (!biomesPatched) {
				for (Biome biome : this.biomes) {
					biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
						{
							carvableBlocks = ImmutableSet.of(DreamStoneBlock.block.getDefaultState().getBlock(),
